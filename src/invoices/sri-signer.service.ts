...
export class SriSignerService {
  ...
  generateInvoiceXml(data: InvoiceData): string {
    ...
    return (
      ...
      `<tipoIdentificacionComprador>05</tipoIdentificacionComprador>` + // 05 = Cédula, 04 = RUC, 07 = Consumidor Final
      `<razonSocialComprador>${this.escapeXml(data.clientName)}</razonSocialComprador>` +
      `<identificacionComprador>1713567890</identificacionComprador>` + // Default simulated comprador identification
  ...
  // Signs XML content using XAdES-BES
  signXml(xmlContent: string, p12Buffer?: Buffer, passphrase?: string): string {
    let keys: { privateKey: forge.pki.rsa.PrivateKey } | undefined;
    ...
    if (p12Buffer && passphrase) {
      try {
        ...
        keys = { privateKey: keyBag.key as forge.pki.rsa.PrivateKey };
        ...
        console.error(
          'Failed to load PKCS12 certificate, falling back to mock certificate:',
          errMsg,
        );
    ...
    const modulusBase64 = forge.util.encode64(
      forge.util.hexToBytes(modulusHex),
    );
    const exponentHex = rsaPublicKey.e.toString(16);
    const exponentBase64 = forge.util.encode64(
      forge.util.hexToBytes(exponentHex),
    );

    // Certificate issuer distinguished name
    const issuerName = cert.issuer.attributes
      .map(
        (attr: { name: string; value: string; shortName?: string }) =>
          `${attr.shortName || attr.name.toUpperCase()}=${attr.value}`,
      )
      .join(', ');
    ...
    // 2. Build KeyInfo XML
    const keyInfoXml =
      `<ds:KeyInfo Id="${certificateKeyInfoId}">` +
      `<ds:X509Data>` +
    ...
    return (
      cleanXml.substring(0, closingTagIndex) +
      signatureXml +
  ...
  // Helpers
  private sha1Base64(data: string): string {
    const md = forge.md.sha1.create();
    md.update(data, 'utf8');
  ...
  private generateMockP12(): {
    keys: { privateKey: forge.pki.rsa.PrivateKey };
    cert: forge.pki.Certificate;
  } {
    // Generates a mock RSA Keypair and X.509 Certificate in memory for development fallback
    const keys = forge.pki.rsa.generateKeyPair(1024); // 1024-bit for speed in dev fallback
